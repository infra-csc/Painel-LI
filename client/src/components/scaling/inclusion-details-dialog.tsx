...
import { getSaveBlockReason, getConfirmBlockReason, isAtendimentoMissing, isPercurseiroMissing } from "./scaling-validation";
...
export default function InclusionDetailsDialog(props: InclusionDetailsDialogProps) {
  ...
  // Valores Padrão (tabela do percurseiro) — só busca quando a função é de percurso;
  // sem resposta, percurseiroDiariaCents cai nos defaults do shared.
  ...
  const { data: systemSettings } = useQuery<Record<string, number>>({
    ...
    enabled: open && isPercursoInclusion,
    staleTime: 5 * 60 * 1000,
  });
